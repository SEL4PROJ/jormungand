...
lemma 
  ...
  apply (clarsimp simp: pred_conj_def)
  apply (sep_cancel)
  apply (erule sep_curry[where P="list q qs ", rotated])
...
definition list_rev where  
  "list_rev p = do 
     (hd_ptr, rev)  <- whileLoop  (\<lambda>(hd_ptr, rev) s. hd_ptr \<noteq> NULL)
...
lemma list_rev_valid_wp:"
  \<lbrace>list p ps\<rbrace> list_rev p \<lbrace>\<lambda>rv. list rv (rev ps)\<rbrace>"
...
lemma list_rev_valid_sp: 
  ...
  apply (intro seq_ext)
   apply (rule whileLoop_sp_inv; clarsimp)
   apply (intro seq_ext)
     apply (rule get_ptr_sp)
    apply (rule set_ptr_sp')
   apply (rule hoare_strengthen_post[OF return_sp])
   apply (clarsimp)
   apply (clarsimp simp: pred_conj_def sep_conj_exists sep_conj_assoc,
          case_tac x; clarsimp simp: pred_conj_def sep_conj_exists sep_conj_assoc)
...
lemma "P s \<Longrightarrow> Q s \<Longrightarrow> (P \<and>* (Q -* (P and Q))) s"
  by (metis disjoint_zero_sym pred_andI pred_neg_def sep_add_zero_sym sep_conjI
